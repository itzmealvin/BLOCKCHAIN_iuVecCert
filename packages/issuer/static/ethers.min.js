// deno-lint-ignore-file -- JavaScript library
const __$G = typeof globalThis !== "undefined"
  ? globalThis
...
function toBeArray(_value) {
  const value = getUint(_value, "value");
  if (value === BN_0$a) return new Uint8Array([]);
...
class FetchRequest {
  ...
  toString() {
    ...
    } url=${JSON.stringify(this.url)} headers=${
      JSON.stringify(
        this.headers,
      )
  ...
  async #send(attempt, expires, delay, _request, _response) {
    ...
    if (response.statusCode === 301 || response.statusCode === 302) {
      ...
      if (
        ...
        if (
          typeof retryAfter === "string" && retryAfter.match(/^[1-9][0-9]*$/)
        ) {
  ...
  redirect(location) {
    ...
    assert$1(
      ...
      "UNSUPPORTED_OPERATION",
      {
        operation: `redirect(${this.method} ${
          JSON.stringify(
            this.url,
          )
...
class FetchResponse {
  ...
  makeServerError(message, error) {
    ...
    if (!message) {
      ...
      statusMessage =
        `CLIENT ESCALATED SERVER ERROR (${this.statusCode} ${this.statusMessage}; ${message})`;
    }
...
function getFormat(value) {
  ...
  const name = (signed ? "" : "u") + "fixed" + String(width) + "x" +
    String(decimals);
...
function checkOpts(defaults, opts) {
  if (
    opts !== undefined && (typeof opts !== "object" || !isPlainObject(opts))
  ) {
...
class SHA512 extends SHA2 {
  ...
  process(view, offset) {
    ...
    for (let i = 16; i < 80; i++) {
      ...
      const s0h = u64.rotrSH(W15h, W15l, 1) ^
        u64.rotrSH(W15h, W15l, 8) ^
        u64.shrSH(W15h, W15l, 7);
      const s0l = u64.rotrSL(W15h, W15l, 1) ^
        u64.rotrSL(W15h, W15l, 8) ^
        u64.shrSL(W15h, W15l, 7);
      const W2h = SHA512_W_H[i - 2] | 0;
      const W2l = SHA512_W_L[i - 2] | 0;
      const s1h = u64.rotrSH(W2h, W2l, 19) ^
        u64.rotrBH(W2h, W2l, 61) ^
        u64.shrSH(W2h, W2l, 6);
      const s1l = u64.rotrSL(W2h, W2l, 19) ^
    ...
    let { Ah, Al, Bh, Bl, Ch, Cl, Dh, Dl, Eh, El, Fh, Fl, Gh, Gl, Hh, Hl } =
      this;
    for (let i = 0; i < 80; i++) {
      const sigma1h = u64.rotrSH(Eh, El, 14) ^
        u64.rotrSH(Eh, El, 18) ^
        u64.rotrBH(Eh, El, 41);
      const sigma1l = u64.rotrSL(Eh, El, 14) ^
        u64.rotrSL(Eh, El, 18) ^
        u64.rotrBL(Eh, El, 41);
      ...
      const T1l = T1ll | 0;
      const sigma0h = u64.rotrSH(Ah, Al, 28) ^
        u64.rotrBH(Ah, Al, 34) ^
        u64.rotrBH(Ah, Al, 39);
      const sigma0l = u64.rotrSL(Ah, Al, 28) ^
        u64.rotrBL(Ah, Al, 34) ^
        u64.rotrBL(Ah, Al, 39);
...
class Point {
  ...
  multiplyAndAddUnsafe(Q, a, b) {
    ...
    const aP = a === _0n || a === _1n || this !== Point.BASE
      ? P.multiplyUnsafe(a)
      : P.multiply(a);
...
let multi = new Set();
for (let g of GROUPS) {
  for (let cp of g.V) (union.has(cp) ? multi : union).add(cp);
}
for (let cp of union) {
  if (!WHOLE_MAP.has(cp) && !multi.has(cp)) WHOLE_MAP.set(cp, UNIQUE_PH);
}
...
function determine_group(unique) {
  ...
  for (let cp of unique) {
    let gs = groups.filter((g) => g.V.has(cp));
    if (!gs.length) {
      if (groups === GROUPS) throw error_disallowed(cp);
...
function check_group(g, cps) {
  let { V, M } = g;
  for (let cp of cps) if (!V.has(cp)) throw error_group_member(g, cp);
  if (M) {
...
class Transaction {
  ...
  get maxPriorityFeePerGas() {
    ...
    if (value == null) {
      if (this.type === 2) return BN_0$4;
      return null;
    }
    return value;
  }
  set maxPriorityFeePerGas(value) {
    this.#maxPriorityFeePerGas = value == null
      ? null
      : getBigInt(value, "maxPriorityFeePerGas");
  }
  get maxFeePerGas() {
  ...
  set maxFeePerGas(value) {
    this.#maxFeePerGas = value == null
      ? null
      : getBigInt(value, "maxFeePerGas");
  }
  get data() {
    return this.#data;
  }
  ...
  inferTypes() {
    ...
    const hasFee = this.maxFeePerGas != null ||
      this.maxPriorityFeePerGas != null;
  ...
  while (offset < text.length) {
    ...
    if (type) {
      ...
      if (type === "OPEN_PAREN") {
        ...
        if (
          tokens.length === 0 || tokens[tokens.length - 1].type !== "BRACKET"
        ) {
...
class Interface {
  ...
  #getFunction(key, values, forceUnique) {
    ...
    if (key.indexOf("(") === -1) {
      ...
      for (const [name, fragment] of this.#functions) {
        if (name.split("(")[0] === key) matching.push(fragment);
      }
      if (values) {
  ...
  #getEvent(key, values, forceUnique) {
    ...
    if (key.indexOf("(") === -1) {
      ...
      if (values) {
        for (let i = matching.length - 1; i >= 0; i--) {
          if (matching[i].inputs.length < values.length) matching.splice(i, 1);
        }
        for (let i = matching.length - 1; i >= 0; i--) {
          const inputs = matching[i].inputs;
          for (let j = 0; j < values.length; j++) {
  ...
  decodeEventLog(fragment, data, topics) {
    ...
    const resultIndexed = topics != null
...
class Block {
  ...
  [Symbol.iterator]() {
    ...
    return {
      next: () => {
        if (index < this.length) return { value: txs[index++], done: false };
        return { value: undefined, done: true };
      },
    };
  }
...
class TransactionResponse {
  ...
  constructor(tx, provider) {
    ...
    this.maxPriorityFeePerGas = tx.maxPriorityFeePerGas != null
...
async function copyOverrides(arg, allowed) {
  ...
  if (overrides.from) overrides.from = await resolveAddress(overrides.from);
  return overrides;
}
...
class BaseContract {
  ...
  constructor(target, abi, runner, _deployTx) {
    ...
    defineProperties(this, {
      fallback: iface.receive || iface.fallback
  ...
  computeIntrinsicGas(tx) {
    ...
    if (tx.data) {
      for (let i = 2; i < tx.data.length; i += 2) {
        if (tx.data.substring(i, i + 2) === "00") gas += costs.txDataZero;
...
class AbstractSigner {
  ...
  async populateTransaction(tx) {
    ...
    const hasEip1559 = pop.maxFeePerGas != null ||
...
function _spelunkMessage(value, result) {
  ...
  if (typeof value === "object") {
    for (const key in value) _spelunkMessage(value[key], result);
  }
  if (typeof value === "string") {
    try {
  ...
  async fetch(module, params, post) {
    ...
    request.processFunc = async (request, response) => {
      const result = response.hasBody()
        ...
        : {};
      const throttle = (typeof result.result === "string" ? result.result : "")
        .toLowerCase()
...
function checkQuorum(quorum, results) {
  ...
  for (const r of tally.values()) {
    if (r.weight >= quorum && (!best || r.weight > best.weight)) best = r;
  }
...
const Testnets =
  "goerli kovan sepolia classicKotti optimism-goerli arbitrum-goerli matic-mumbai bnbt"
    .split(
      " ",
    );
...
function mnemonicToEntropy(mnemonic, wordlist) {
  ...
  const checksum = getBytes(sha256(entropy.slice(0, entropyBits / 8)))[0] &
    checksumMask;
  assertArgument(
...
function entropyToMnemonic(entropy, wordlist) {
  ...
  const checksum = parseInt(sha256(entropy).substring(2, 4), 16) &
...
var __classPrivateFieldGet$2 = (__$G && __$G.__classPrivateFieldGet) ||
...
var __classPrivateFieldSet$2 = (__$G && __$G.__classPrivateFieldSet) ||
...
class AES {
  ...
  constructor(key) {
    ...
    while (t < roundKeyCount) {
      ...
      rconpointer += 1;
      if (KC != 8) {
        for (let i = 1; i < KC; i++) tk[i] ^= tk[i - 1];
      } else {
...
var __classPrivateFieldSet$1 = (__$G && __$G.__classPrivateFieldSet) ||
  function (receiver, state, value, kind, f) {
    if (kind === "m") throw new TypeError("Private method is not writable");
    if (kind === "a" && !f) {
      throw new TypeError("Private accessor was defined without a setter");
...
var __classPrivateFieldGet$1 = (__$G && __$G.__classPrivateFieldGet) ||
  function (receiver, state, kind, f) {
    if (kind === "a" && !f) {
      throw new TypeError("Private accessor was defined without a getter");
    }
...
var __classPrivateFieldSet = (__$G && __$G.__classPrivateFieldSet) ||
  function (receiver, state, value, kind, f) {
    if (kind === "m") throw new TypeError("Private method is not writable");
    if (kind === "a" && !f) {
      throw new TypeError("Private accessor was defined without a setter");
...
var __classPrivateFieldGet = (__$G && __$G.__classPrivateFieldGet) ||
  function (receiver, state, kind, f) {
    if (kind === "a" && !f) {
      throw new TypeError("Private accessor was defined without a getter");
    }
